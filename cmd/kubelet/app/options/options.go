...
func (s *KubeletServer) AddFlags(fs *pflag.FlagSet) {
	...
	fs.BoolVar(&s.RunOnce, "runonce", s.RunOnce, "If true, exit after spawning pods from local manifests or remote urls")
	fs.DurationVar(&s.RunOnceTimeout, "runonce-timeout", s.RunOnceTimeout, "Duration of time to wait before exiting when in runonce mode")
	...
	fs.Var(&bindableBootstrapPodLabel, "bootstrap-pod-label", "Label used to filter pods when looking for bootstrap pod.")
