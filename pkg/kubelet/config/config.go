...
func (s *podStorage) Merge(source string, change interface{}) error {
	...
	case PodConfigNotificationSnapshot:
		if len(updates.Pods) > 0 || len(deletes.Pods) > 0 || len(adds.Pods) > 0 {
			fmt.Println("MERGED STATE --------------------------- ", len(updates.Pods), firstSet, s.MergedState())
			s.updates <- kubetypes.PodUpdate{Pods: s.MergedState().([]*api.Pod), Op: kubetypes.SET, Source: source}
		} else if firstSet {
			s.updates <- kubetypes.PodUpdate{Pods: s.MergedState().([]*api.Pod), Op: kubetypes.SOURCE_READY, Source: source}
		}
